...
// Location: _GameScreenState class
...
class _GameScreenState extends State<GameScreen> {
  ...
  Size? _lastScreenSize; // Still track size for initial generation
  bool? _lastThemeIsDark;
  ColorPalette? _lastPaletteUsed;

  // Removed Bokeh animation controller
  late ConfettiController _confettiController;

  // --- Method: Regenerate Bokeh Particles (Unchanged) ---
  ...
  // --- Updated _updateBokehIfNeeded: Removed Size Check ---
  void _updateBokehIfNeeded() {
     ...
     final settings = Provider.of<SettingsProvider>(context, listen: false);
     final currentSize = mediaQueryData.size; // Still need current size for generation if needed
     final currentThemeData = appThemes[settings.selectedThemeKey] ?? appThemes[lightThemeKey]!;
     final currentThemeIsDark = currentThemeData.brightness == Brightness.dark;
     final currentPalette = settings.selectedPalette;

     // Determine if an update is needed based only on initialization status, theme, or palette change
     bool needsUpdate = !_particlesInitialized ||
                         // --- REMOVED size check: currentSize != _lastScreenSize || ---
                         currentThemeIsDark != _lastThemeIsDark ||
                         currentPalette != _lastPaletteUsed;

     if (needsUpdate) {
        // Call regeneration only if not initialized or theme/palette changed
        // We also update _lastScreenSize here so the *next* call to createBokehParticles
        // inside _regenerateBokehParticles uses the *latest* size if it was regenerated
        // due to theme/palette change or initialization.
        if(!_particlesInitialized) {
          _lastScreenSize = currentSize; // Capture size on first generation
        }
        _regenerateBokehParticles();
     } else if (_lastScreenSize == null && _particlesInitialized) {
         // Edge case: If initialized but size wasn't captured (e.g. very fast init), capture it now.
         // This prevents _regenerateBokehParticles using a null size if called later.
  ...
  // --- Methods: initState, didChangeDependencies, dispose, _formatDuration, _showSettingsSheet, _showCompletionDialog ---
  // (These remain unchanged)
   @override
  void initState() { super.initState(); _confettiController = ConfettiController(duration: const Duration(seconds: 2)); }
   @override
  ...
  // --- Build Method (Unchanged from previous version) ---
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar( backgroundColor: currentTheme.brightness == Brightness.dark ? Colors.black.withOpacity(0.2) : Colors.white.withOpacity(0.1), elevation: 0, foregroundColor: currentTheme.colorScheme.onSurface,
