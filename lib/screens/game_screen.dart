...
// Location: Entire File (Moved Difficulty Chip from AppBar to Body Stack)
...
class _GameScreenState extends State<GameScreen> {
  ...
  // --- Methods (initState, didChangeDependencies, dispose, regeneration, updates, format, dialogs - unchanged) ---
  @override void initState() { super.initState(); _confettiController = ConfettiController(duration: const Duration(seconds: 2)); }
  @override void didChangeDependencies() { super.didChangeDependencies(); _updateBokehIfNeeded(); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
         ...
         title: RichText( /* ... Rainbodoku Title ... */ text: TextSpan( style: baseTitleStyle, children: <TextSpan>[ TextSpan(text: 'R', style: TextStyle(color: titleColors[1])), TextSpan(text: 'a', style: TextStyle(color: titleColors[2])), TextSpan(text: 'i', style: TextStyle(color: titleColors[0])), TextSpan(text: 'n', style: TextStyle(color: titleColors[3])), TextSpan(text: 'b', style: TextStyle(color: titleColors[4])), TextSpan(text: 'o', style: TextStyle(color: titleColors[5])), const TextSpan(text: 'doku'), ], ), ),
         leading: IconButton( icon: const Icon(Icons.arrow_back), onPressed: () { gameProvider.pauseGame(); Navigator.pop(context); }, ),
         actions: [
             // --- REMOVED Difficulty Chip from AppBar actions ---
             // Consumer<GameProvider>( ... )

            // Pause/Play Button & Dialog Trigger (Unchanged)
            Selector<GameProvider, Tuple2<bool, bool>>( selector: (_, game) => Tuple2(game.isPaused, game.isCompleted), builder: (context, data, child) { final isPaused = data.item1; final isCompleted = data.item2; if (isCompleted && !_completionDialogShown) { WidgetsBinding.instance.addPostFrameCallback((_) { if(mounted && isCompleted && !_completionDialogShown) { final finalTime = Provider.of<GameProvider>(context, listen: false).elapsedTime; setState(() { _completionDialogShown = true; }); _showCompletionDialog(context, finalTime); } }); } return IconButton( icon: Icon(isPaused ? Icons.play_arrow : Icons.pause, color: isCompleted ? Colors.grey : null), tooltip: isPaused ? 'Resume' : 'Pause', onPressed: isCompleted ? null : () { final game = Provider.of<GameProvider>(context, listen: false); if (game.isPaused) { game.resumeGame(); } else { game.pauseGame(); } }, ); } ),
      ...
      body: Stack(
        children: [
            ...
            // Layer 4: Main Game Content Area (SafeArea)
            SafeArea(
              // Add minimum padding to keep chip off edge
              minimum: const EdgeInsets.only(top: 8.0, right: 8.0),
              child: Stack( // Use Stack to overlay Chip on top of Column
                children: [
                  // Original Column containing Timer, Grid, Controls Area
                  Padding(
                    padding: const EdgeInsets.only(left: 12.0, right: 12.0, bottom: 12.0, top: 0), // Adjust padding if needed
                    child: Column(
                      children: <Widget>[
                          ...
                          Stack( alignment: Alignment.center, children: [
                              Visibility( visible: !gameProvider.isCompleted, maintainState: true, maintainAnimation: true, maintainSize: true, child: Column( mainAxisSize: MainAxisSize.min, children: const [ PaletteSelectorWidget(), SizedBox(height: 15), GameControls(), ], ), ),
                  ...
                  ), // End Original Column Padding

                  // --- NEW: Difficulty Chip Overlay ---
                  // Positioned in top right corner of the SafeArea content
                  Align(
                     alignment: Alignment.topRight,
                     child: Padding(
                        // Add padding from the safe area edges
                        ...
                        child: Consumer<GameProvider>(
                          builder: (context, game, child) {
                            ...
                            // Use the same Chip widget as before
                            return Chip(
                              label: Text(
                                difficultyText,
                  ...
                  // --- END NEW Difficulty Chip Overlay ---

                ], // End Main Stack children (Column + Chip Overlay)
              ), // End Stack inside SafeArea
            ), // End SafeArea
